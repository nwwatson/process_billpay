...
module BluePay
  class Client
    ...
    include BluePay::ApiRequest
    include BluePay::ApiResponse

    def initialize(params = {})
      @ACCOUNT_ID = params[:account_id] || Rails.application.credentials.bluepay[:account_id]
      @SECRET_KEY = params[:secret_key] || Rails.application.credentials.bluepay[:secret_key]
      @PARAM_HASH = { 'MODE' => (params[:mode] || "LIVE") }
    end
    ...
    def auth(params = {})
      ...
      if params[:new_customer_token] && params[:new_customer_token] != false
        @PARAM_HASH['NEW_CUST_TOKEN'] =
          params[:new_customer_token] == true ? SecureRandom.hex(8) : params[:new_customer_token]
      end
    end
    ...
    def query_by_payment_type(_pay_type)
    ...
    def set_hash_type(chosen_hash)
      ...
      %w[MD5 SHA256 SHA512 HMAC_SHA256].include?(chosen_hash.upcase) ? chosen_hash.upcase : default_hash
    end
    ...
    def set_bp10emu_tps_string
      ...
      add_string_protected_status(bp10emu)
    end
    ...
    def set_shpf_tps_string
      ...
      add_string_protected_status(shpf)
    end
    ...
    def set_receipt_url
      if @PARAM_HASH['RECEIPT_FORM_ID'] == 'remote_url'
        @PARAM_HASH['REMOTE_URL']
      else
        'https://secure.bluepay.com/interfaces/shpf?SHPF_FORM_ID=' + @PARAM_HASH['RECEIPT_FORM_ID'] +
    ...
    def add_string_protected_status(string)
      ...
      if @PARAM_HASH['REB_PROTECT'] == 'Yes'
        string.concat([@PARAM_HASH['REBILLING'], @PARAM_HASH['REB_CYCLES'], @PARAM_HASH['REB_AMOUNT'],
                       @PARAM_HASH['REB_EXPR'], @PARAM_HASH['REB_FIRST_DATE']].join(''))
      end
